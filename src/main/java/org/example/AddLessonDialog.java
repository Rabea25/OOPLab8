...
public class AddLessonDialog extends JDialog{
    ...
    public AddLessonDialog(Course course, CourseService courseService) {
        ...
        saveButton.addActionListener(e -> {
            ...
            if (!Validations.isNonEmpty(qTitle) || !Validations.isValidTitle(qTitle)) {
                JOptionPane.showMessageDialog(root,"Quiz title cannot be empty, must be at least 4 characters","Invalid Title", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            String newLessonId = courseService.addLesson(course.getCourseId(), title, content, Arrays.asList(res), qTitle, questions, options, answers);
            if(newLessonId != null) {
